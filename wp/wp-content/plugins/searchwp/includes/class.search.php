...
class SearchWPSearch {
	...
	function queryForPostIDs() {
		...
		foreach( $this->terms as $term ) {
			...
			// prep the term
			$prepped_term           = $this->prep_term( $term );
		...
		$this->sql .= $this->query_limit_pool_by_stem( $this->terms );
	...
	function query_limit_pool_by_stem() {
		...
		if ( is_array( $this->terms_final ) && count ( $this->terms_final ) ) {
			...
			$this->sql .= " AND ( {$this->db_prefix}terms.term IN ('" . implode( "','", $this->terms_final ) . "') OR {$this->db_prefix}terms.stem IN ('" . implode( "','", $closing_stems ) . "') ) ";
		}
